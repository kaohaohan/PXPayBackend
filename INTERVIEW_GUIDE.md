# 🎯 E-Commerce Backend - 面試技術說明

## 專案亮點 (Key Achievements)

### High-Concurrency Optimization

- 使用 JMeter 進行壓力測試：100 併發用戶 × 100,000 筆資料
- 實作三層優化架構：Database Indexing + Redis Cache + Connection Pooling

### 量化成果 (Quantitative Results)

#### 1️⃣ Database Indexing 優化

- **錯誤率**: 從 80% 降至 0% (Zero downtime)
- **吞吐量**: 從 7.5 req/min 提升至 44.8 req/sec (提升 ~300 倍)
- **平均回應時間**: 從 9.5 秒降至 2.2 秒 (改善 76%)
- **時間複雜度**: O(n) → O(log n)

#### 2️⃣ Redis Cache 優化

- **Cache Miss**: 319ms (首次查詢)
- **Cache Hit**: 58-60ms (後續查詢)
- **效能提升**: 5.3-5.5 倍
- **快取策略**: Cache-Aside Pattern，5 分鐘過期

---

## 🏗️ 技術架構

### 三層優化架構

```
┌─────────────────────────────────────────┐
│  Layer 1: Database Indexing             │
│  - B-Tree Index on Product.Name         │
│  - StartsWith() → LIKE 'keyword%'       │
│  - Time Complexity: O(log n)            │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│  Layer 2: Redis Distributed Cache       │
│  - Cache-Aside Pattern                  │
│  - 5 min TTL (Time To Live)             │
│  - Time Complexity: O(1)                │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│  Layer 3: Connection Pooling            │
│  - Min Pool Size: 10                    │
│  - Max Pool Size: 200                   │
│  - Prevents Socket Exhaustion           │
└─────────────────────────────────────────┘
```

---

## 📊 API 端點比較

### 1. `/api/products/search/{name}` - 無優化版本

- **方法**: Contains() → `LIKE '%keyword%'`
- **索引**: ❌ 無法使用
- **快取**: ❌ 無
- **效能**: 200-300ms (全表掃描)
- **適用**: 需要搜尋任意位置的關鍵字

### 2. `/api/products/search-starts-with/{name}` - 索引優化版本

- **方法**: StartsWith() → `LIKE 'keyword%'`
- **索引**: ✅ B-Tree Index
- **快取**: ❌ 無
- **效能**: 1-5ms (索引查詢)
- **適用**: 搜尋名稱開頭的關鍵字

### 3. `/api/products/search-cached/{name}` - Redis 快取版本

- **方法**: StartsWith() → `LIKE 'keyword%'`
- **索引**: ✅ B-Tree Index
- **快取**: ✅ Redis (5 min TTL)
- **效能**:
  - Cache Miss: 319ms
  - Cache Hit: 58-60ms
- **適用**: 熱門商品搜尋、高流量 API

---

## 🔍 技術深度說明

### 為什麼 Contains() 無法使用索引？

```sql
-- Contains() 轉換成 SQL
WHERE Name LIKE '%全聯先生%'
```

**問題**: `%` 在前面會導致：

1. 資料庫無法使用 B-Tree 索引
2. 必須進行全表掃描 (Full Table Scan)
3. 時間複雜度: O(n)

### 為什麼 StartsWith() 可以使用索引？

```sql
-- StartsWith() 轉換成 SQL
WHERE Name LIKE '全聯先生%'
```

**優勢**: 沒有前置 `%`，可以：

1. 使用 B-Tree 索引進行前綴搜尋
2. 只掃描符合前綴的資料
3. 時間複雜度: O(log n)

### B-Tree 索引原理

```
         [全]
        /   \
    [全家]  [全聯]
           /    \
      [全聯先生] [全聯福利中心]
```

- 類似二元搜尋樹
- 可以快速定位前綴
- 查詢效率: O(log n)

---

## 🚀 Redis Cache-Aside Pattern

### 工作流程

```
1. 收到請求 → 查詢 Redis
   ├─ Cache Hit → 直接返回 (58-60ms)
   └─ Cache Miss → 查詢資料庫 (319ms)
                 → 寫入 Redis
                 → 返回結果
```

### 為什麼選擇 Cache-Aside？

1. **簡單易懂**: 邏輯清晰，容易維護
2. **適合讀多寫少**: 電商搜尋就是讀多寫少
3. **容錯性好**: Redis 掛掉還能從 DB 讀取
4. **業界標準**: 最常見的快取策略

---

## 🔧 Connection Pooling 解決的問題

### 問題：Socket Exhaustion (Socket 耗盡)

**症狀**:

```
java.net.SocketException: Socket closed
Error Rate: 80%
```

**原因**:

- 高併發下，每個請求都建立新的 DB 連線
- 連線建立/關閉很慢 (TCP 三次握手)
- 系統 Socket 數量有限

**解決方案**:

```csharp
"Min Pool Size=10;Max Pool Size=200;Connection Timeout=30;"
```

- **Min Pool Size**: 預先建立 10 個連線
- **Max Pool Size**: 最多 200 個連線
- **Connection Timeout**: 30 秒超時

---

## 📈 JMeter 壓力測試

### 測試設定

- **Thread Count**: 100 (併發用戶)
- **Ramp-Up Period**: 1 秒
- **Loop Count**: 1
- **資料量**: 100,000 筆商品

### 測試結果對比

| 指標         | 無索引  | 有索引   | 有索引+Redis          |
| ------------ | ------- | -------- | --------------------- |
| 錯誤率       | 80%     | 0%       | 0%                    |
| 平均回應時間 | 9.5s    | 2.2s     | 0.06s (Cache Hit)     |
| 吞吐量       | 7.5/min | 44.8/sec | ~1000/sec (Cache Hit) |

---

## 💬 面試時可以這樣說

### 1. 介紹專案背景

> "我做了一個電商後端 API，主要處理商品搜尋功能。在壓力測試時發現，當 100 個用戶同時搜尋 10 萬筆商品時，錯誤率高達 80%，這是不可接受的。"

### 2. 說明問題分析

> "我用 JMeter 進行壓力測試，發現主要有兩個問題：
>
> 1. 資料庫查詢太慢 - 因為使用 Contains() 無法利用索引
> 2. 高併發下出現 Socket Exhaustion - 因為沒有設定 Connection Pool"

### 3. 說明解決方案

> "我實作了三層優化：
>
> **第一層：Database Indexing**
>
> - 在 Product.Name 欄位建立 B-Tree 索引
> - 將查詢從 Contains() 改為 StartsWith()
> - 時間複雜度從 O(n) 降至 O(log n)
>
> **第二層：Redis Cache**
>
> - 使用 Cache-Aside Pattern
> - 熱門查詢從 Redis 讀取，快 5 倍
> - 設定 5 分鐘 TTL，避免資料過期
>
> **第三層：Connection Pooling**
>
> - 設定 Min Pool Size 10, Max Pool Size 200
> - 避免高併發下的 Socket Exhaustion"

### 4. 說明成果

> "最終結果：
>
> - 錯誤率從 80% 降至 0%
> - 吞吐量提升 300 倍
> - 平均回應時間改善 76%
> - Cache Hit 情況下，回應時間只要 60ms"

### 5. 展現學習能力

> "這個專案讓我學到：
>
> 1. 索引的重要性 - 不是所有查詢都能用索引
> 2. 快取策略 - Cache-Aside 適合讀多寫少的場景
> 3. 系統調優 - Connection Pool 可以大幅提升併發能力
> 4. 測試驗證 - JMeter 可以量化優化效果"

---

## 🎓 可能的面試問題

### Q1: 為什麼不用 Full-Text Search？

**A**:

- Full-Text Search 更適合複雜的文字搜尋（如搜尋引擎）
- 我們的場景是簡單的前綴搜尋，B-Tree Index 就夠了
- B-Tree Index 更輕量，維護成本更低

### Q2: Redis 掛掉怎麼辦？

**A**:

- Cache-Aside Pattern 的優勢：Redis 掛掉還能從 DB 讀取
- 可以加 try-catch，Redis 失敗時直接查 DB
- 生產環境會用 Redis Cluster 做高可用

### Q3: 為什麼 Cache 設定 5 分鐘過期？

**A**:

- 平衡資料新鮮度和效能
- 商品資料不常變動，5 分鐘是合理的
- 如果商品更新，可以手動清除 Cache

### Q4: 如果要支援模糊搜尋怎麼辦？

**A**:

- 可以用 Elasticsearch 或 Full-Text Search
- 或者用 N-gram 索引
- 或者前端用 StartsWith，後端用 Contains 做二次過濾

### Q5: 100 併發算高嗎？

**A**:

- 對小型電商來說算高了
- 大型電商可能上千上萬併發
- 但優化思路是一樣的：索引 + 快取 + 連線池

---

## 🔗 GitHub 連結

**專案網址**: https://github.com/kaohaohan/E-Commerce-Backend

**重點 Commit**:

- Database Indexing: `docs: 新增詳細的效能優化註解和技術說明`
- Redis Cache: `feat: 新增 Redis 分散式快取支援`
- Bug Fix: `fix: 修正 Redis Cache Hit 判斷邏輯`

---

## ⏰ Demo 流程（面試時）

### 1. 展示三個 API (5 分鐘)

```bash
# 1. 無優化版本 (Contains)
curl http://localhost:5000/api/products/search/全聯先生
# 結果: 200-300ms

# 2. 索引優化版本 (StartsWith + Index)
curl http://localhost:5000/api/products/search-starts-with/全聯先生
# 結果: 1-5ms

# 3. Redis 快取版本 (StartsWith + Index + Redis)
curl http://localhost:5000/api/products/search-cached/全聯先生
# 第一次: 319ms (Cache Miss)
# 第二次: 58ms (Cache Hit)
```

### 2. 展示程式碼 (3 分鐘)

- `Product.cs`: 展示 `[Index(nameof(Name))]`
- `ProductService.cs`: 展示 `Contains()` vs `StartsWith()`
- `ProductService.cs`: 展示 Redis Cache-Aside Pattern
- `appsettings.json`: 展示 Connection Pool 設定

### 3. 展示 JMeter 測試結果 (2 分鐘)

- 展示 Summary Report
- 說明錯誤率、吞吐量、回應時間的改善

---

## 📚 延伸學習

如果面試官問「還可以怎麼優化？」：

1. **Read Replica**: 讀寫分離，讀取用 Slave DB
2. **CDN**: 靜態資源用 CDN 加速
3. **Load Balancer**: 多台伺服器負載均衡
4. **Message Queue**: 非同步處理，減少回應時間
5. **Database Sharding**: 資料分片，水平擴展

---

## 🎯 總結

這個專案展現了我：

1. ✅ **問題分析能力** - 用 JMeter 找出瓶頸
2. ✅ **技術實作能力** - 實作 Index + Redis + Connection Pool
3. ✅ **效能優化能力** - 錯誤率從 80% 降至 0%
4. ✅ **量化思維** - 用數據證明優化效果
5. ✅ **學習能力** - 從問題中學習新技術

**適合職位**: Full Stack Developer, Backend Engineer, 需要處理高流量的電商/社群平台

---

祝面試順利！🚀
